...
class BridgeHandEditor:
    ...
    def export_current_deal(self):
        ...
        def format_cards(hand_data):
            cards = ",".join(card["card"] for card in hand_data)
            return [cards] + ["" for _ in range(16)]

        def format_sequences(hand_data):
            seq = ",".join(card["sequence"] for card in hand_data)
            return [seq] + ["" for _ in range(16)]

        def format_bidding(bids):
            bids_str = ",".join(bids)
            return [bids_str] + ["" for _ in range(16)]
