...
def updatedealinhandsfromai(handsfromaipath, targetgroup, targetdeal, newdatalines):
    ...
    # Now insert the 8 new lines from HandsTemp
...
def validate_deal_data(group, deal, datalines, validation_errors):
    """
    Validate one deal's 8 data rows from HandsTemp (already parsed by csv.reader).
    ...
    if len(datalines) != 8:
        validation_errors.append(
            f"Group {group}, Deal {deal}: Expected 8 data lines, got {len(datalines)}"
        )
        return False
    ...
    for i, idx in enumerate(card_indices):
        ...
        for raw_card in row:
            ...
            if card in card_owner:
                ...
                descr = f"{card} ({first} and {directions[i]})"
    ...
    if len(card_owner) != 52:
        if duplicate_details:
            ...
            validation_errors.append(
                f"Group {group}, Deal {deal}: Duplicates - {dup_text}"
            )
        else:
            validation_errors.append(
                f"Group {group}, Deal {deal}: Only {len(card_owner)} unique cards (missing {52 - len(card_owner)})"
            )

    # ----- SEQUENCE VALIDATION -----
    seq_indices = [1, 3, 5, 7]  # N, E, S, W sequences
    seq_directions = ["North", "East", "South", "West"]
    ...
    for i, idx in enumerate(seq_indices):
        ...
        try:
            ...
            validation_errors.append(
                f"Group {group}, Deal {deal}: Invalid numbers in {seq_directions[i]} sequence"
            )
            continue
        ...
        if actual != expected:
            ...
            validation_errors.append(
                f"Group {group}, Deal {deal}: {seq_directions[i]} sequence - {', '.join(parts)}"
            )
...
def show_validation_summary(validation_errors):
    """Display validation errors in a scrollable popup window with copy-to-clipboard button"""
    if not validation_errors:
        messagebox.showinfo("Validation Complete", "All deals passed validation!")
        return

    root = tk.Tk()
    root.title("Deal Validation Summary")
    root.geometry("900x700")
    root.resizable(True, True)

    # Scrollable text area
    text_area = scrolledtext.ScrolledText(
        root, wrap=tk.WORD, width=100, height=35, font=("Consolas", 10)
    )
    ...
    def copy_to_clipboard():
        ...
        messagebox.showinfo("Copied", "Validation summary copied to clipboard!")
...
def processhandstemprecordbyrecord(handstemppath, handsfromaipath):
    """Process HandsTemp.csv record by record, updating HandsFromAI.csv."""
    processedcount = 0
    skippedcount = 0
    errorcount = 0
    totalrows = 0
    validation_errors = []  # NEW: Track validation issues

    print("Processing HandsTemp.csv record by record...")
    ...
    with open(handstemppath, "r", newline="") as file:
        ...
        for row in reader:
            ...
            if dealkey:
                ...
                # NEW: Validate deal data BEFORE processing
    ...
    print(f"Total rows read from HandsTemp: {totalrows}")

    # NEW: Show validation summary popup
    print("=" * 60)
...
def main():
    ...
    Optional --base-dir - Resolves ...Group{group}/CSV/HandsTemp.csv,HandsFromAI.csv
    """
    print("CSV Deal Update Script - Record-by-Record, non-interactive")
    print("=" * 60)

    parser = argparse.ArgumentParser(
        description="Update HandsFromAI.csv with data from HandsTemp.csv for a given group."
    )
    parser.add_argument(
        "--group",
        "-g",
