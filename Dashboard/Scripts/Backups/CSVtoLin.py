...
def organize_bids_in_rotation(biddingdatadict, dealerposchar):
    ...
    max_rounds = (
        max(len(bids) for bids in biddingdatadict.values() if isinstance(bids, list))
        if biddingdatadict
        else 0
    )

    for roundidx in range(max_rounds):
        for player in rotation:
            playerbids = biddingdatadict.get(player, [])
            if roundidx < len(playerbids):
                organizedbids.append(playerbids[roundidx])
...
def read_deals_from_ai_csv(group_csv_path):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            current_deal_lines.append(line)
            if (i + 1) % 13 == 0:
                if is_deal_fully_populated(current_deal_lines):
                    all_deals_data.append(list(current_deal_lines))
                    header = current_deal_lines[0].split(",")
                    if len(header) > 3:
                        deal_numbers.append(header[3])
                current_deal_lines.clear()
    except Exception as e:
        return [], []
    return deal_numbers, all_deals_data
...
class CSVtoLinApp:
    ...
    def load_deals(self):
        ...
        nums, self.all_deals_data = read_deals_from_ai_csv(path)
        self.cb["values"] = nums
        if nums:
            self.selected_deal_num.set(nums[0])
        self.status_var.set(f"Loaded {len(nums)} deals.")
