...
# CONFIG
API_KEY = "AIzaSyDrGmKznc74I1NDkGnGNee3BP7-pIfUI6g"
...
MODE_CONFIG = {
    "deals": {
        "folder": "Deals",
        "prompt_file": "TranscribeDealsFromPDF.txt",
        "label": "Deals",
    },
    "intro": {
        "folder": "Intro",
...
def run_transcription(group_num: str, pdf_path: str, prompts_folder: str, mode: str):
    ...
    # ── Pre-flight check: make sure Booklet.pdf is not locked by a viewer ────
    ...
    if os.path.exists(output_pdf):
        try:
            ...
            print(
                f"ERROR: {output_pdf} is open in another program (e.g. PDFGear). "
                "Please close it and try again."
            )
            sys.exit(1)
        print("Pre-flight check passed: Booklet.pdf is not locked.")
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        print(f"Generating transcription (attempt {attempt} of {MAX_ATTEMPTS})...")
        ...
        try:
            ...
            if attempt < MAX_ATTEMPTS:
                ...
                print("ERROR: All attempts failed due to API errors.")
                sys.exit(1)

        if not response.text:
            print(f"  Attempt {attempt}: empty response from Gemini.")
            if attempt < MAX_ATTEMPTS:
                print("  Retrying...")
                continue
            else:
        ...
        print(f"  Received {len(candidate_text)} chars")

        # ── Completeness checks ───────────────────────────────────────────────

        # Check 1: finish_reason from the API
        finish_reason = None
        try:
        ...
        truncated_by_api = finish_reason and str(finish_reason) in ("MAX_TOKENS", "2")

        if truncated_by_api:
            print(
                f"  ✗ Attempt {attempt}: API reports output was cut off (MAX_TOKENS)."
            )
        else:
            print(f"  ✓ Attempt {attempt}: transcription appears complete.")
        ...
        if attempt < MAX_ATTEMPTS:
            ...
            print(
                "ERROR: Transcription incomplete after all attempts. "
                "Check Gemini API status and retry manually."
            )
            sys.exit(1)

    print(f"Total transcription: {len(result_text)} chars")
