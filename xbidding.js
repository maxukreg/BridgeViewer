...
function respondToResize() {
  ...
  availableHeight = window.innerHeight || document.documentElement.clientHeight;
  availableWidth = window.innerWidth || document.documentElement.clientWidth;
  var isMobile = availableWidth < 600;
  ...
  buttonDiv.style.width = totalWidth + 'px'; // ADD 'px' here
  if (showButtonBar()) {
      ...
      if (editorWidthOffset) {
        ...
        buttonDiv.style.left = div.offsetLeft + 'px';
      }
  ...
  if (showAuction()) {
    ...
    const southHandRect = southHand.getBoundingClientRect();
    const auctionHeadingWidth = auctionHeadingDiv.offsetWidth;
    const auctionTableRect = auctionTableDiv.getBoundingClientRect();
    const topPosAucTab = southHandRect.top - auctionTableDiv.offsetHeight - margin;
    const topPosAucHdr = auctionTableRect.top - auctionHeadingDiv.offsetHeight;
    auctionLeft = southHandRect.left + (southHandRect.width / 2) - auctionHeadingWidth;
   
    auctionHeadingDiv.style.left = auctionLeft;
    auctionTableDiv.style.top = `${topPosAucTab}px`;
    auctionHeadingDiv.style.top = `${topPosAucHdr}px`;
    //auctionTableDiv.style.top = auctionTop + auctionHeading.clientHeight;
    auctionTableDiv.style.left = auctionLeft;
    // Add this RIGHT AFTER the auction table positioning block in respondToResize()
    // i.e. after: auctionTableDiv.style.left = auctionLeft;
    
   var biddingBox = document.getElementById('biddingBox');
   if (biddingBox && showBiddingBox()) {
       var boxLeft = auctionLeft + handWidth + margin * 2;
       var boxTop = parseFloat(auctionHeadingDiv.style.top);
       
       biddingBox.style.position = 'absolute';
       biddingBox.style.left = boxLeft + 'px';
       biddingBox.style.top = boxTop + 'px';
       biddingBox.style.width = (totalWidth - boxLeft - margin) + 'px';
       biddingBox.style.height = (5 * suitHeight) + 'px';
...
function hideTricksDiv() {
  tricksDiv.style.visibility = 'hidden';
}
function blankOutHand(seat, blankOut = true) {
  const handDiv = handDivs[seat];
  const nameBar = nameBars[seat];
  
  if (blankOut) {
      // Blank out the hand and name bar
      handDiv.innerHTML = '';
      nameBar.innerHTML = '';
      
      // Set the background color to match the main background
      const mainBackgroundColor = '#006600'; // Assuming the main background color is green (#006600)
      handDiv.style.backgroundColor = mainBackgroundColor;
      nameBar.style.backgroundColor = mainBackgroundColor;
      
      // Remove any borders or outlines
      handDiv.style.border = 'none';
      nameBar.style.border = 'none';
  }
  
  // Remove the mouseover and mouseout event listeners
  nameBar.onmouseover = null;
  nameBar.onmouseout = null;
}

// Call the function to blank out the North, East, and West hands
blankOutHand(2); // North
blankOutHand(3); // East
blankOutHand(1); // West
...
function manageWhosTurn() {
  ...
  if ((whosTurn == 0 || whosTurn == 2) && !isEditor && trick != 12 && seat != 4) {
    ...
    for (suitOrder = suitMin; suitOrder <= suitMax; suitOrder++) {
      ...
      for (card = 12; card >= 0; card--) {
        ...
        if (picturesOfCards) {
          ...
          // Hide the cards for East (seat 3) and West (seat 1) if they are not played
          continue;
...
function manageAnnounceDiv() {
  if (announceDivCentered) {
    ...
    announceDiv.style.fontSize = fontSize*0.8;
  }
...
function showTrickResult() {
  if (displayType == '' && (trick == 12 || tricksClaimed >= 0 || contractLevel == -1)) {
    ...
    undoButton.value = 'Replay';
    claimShowing = true;
    if (!picturesOfCards) {
      wasKibitzed = seatKibitzed;
      setKibitzed('');
    }
    result = result = `${result}<br>Hand complete<br>Click Next for next hand`;
    if (playingClient) {
...
function mouseClickCardImage(cardDiv) {
  ...
  if (canCardBeSelected(suit, card)) {
    ...
    if (
      ...
      nextCard(true, true);
      // if trickwinner is on lead - dont auto
      if ((whosTurn == 1 || whosTurn ==3 ) && trickWinner[trick]!= whosTurn)
      {
        nextCard(true, true);
        if ((whosTurn == 1 || whosTurn ==3 ) && trickWinner[trick]!= whosTurn)
        {
...
function manageBiddingBox() {
  ...
  if (biddingBox) {
    if (showBiddingBox()) {
      ...
      if (lessonID) {
        if (!picturesOfCards) {
          biddingBox.style.width = handWidth + 'px';
        } else {
          biddingBox.style.width = (xpos[3] - (xpos[1] + getHandWidth(1)) - 2 * margin) + 'px';
        }
        biddingBox.style.height = cardImageHeight + 'px';
        biddingBox.style.left = ((totalWidth - biddingBox.clientWidth) / 2) + 'px';
        biddingBox.style.top = ((totalHeight - biddingBox.clientHeight) / 2) + 'px';
        ...
        if (isEditor) {
          biddingBox.style.width = (editorWidthOffset - 2 * margin) + 'px';
        } else {
          biddingBox.style.width = handWidth + 'px';
        }
        if (isEditor) {
          ...
          biddingBox.style.height = handHeight + 'px';
        }

        // Position to the right of auction table, offset by div's left position
        var aucRight = auctionTableDiv.offsetLeft + auctionTableDiv.offsetWidth;
        biddingBox.style.left = (aucRight + margin) + 'px';

        if (isEditor) {
          biddingBox.style.top = (2 * margin + editorAnnounceDiv.clientHeight) + 'px';
        } else {
          biddingBox.style.top = auctionTableDiv.offsetTop + 'px';
        }
      }
      biddingBoxResized();
...
function biddingBoxResized() {
  if (biddingBox) {
    ...
    var widthFactor = 0.137;
    var heightFactor = 0.143;
    var buttonWidth = handWidth * widthFactor;
    var buttonHeight = handHeight * heightFactor;
...
// Example usage:
// resizeButtons(100, 100); // Replace with actual handWidth and handHeight values
...
function documentLoaded() {
  ...
  setTimeout('respondToResize()', 100);
  setTimeout('respondToResize()', 300);
}
...
function positionBiddingBox() {
  ...
  const southHandRect = southHand.getBoundingClientRect();
  const leftPosition = southHandRect.left + (southHandRect.width / 2)+margin;
  // Set the width of the bidding box to twice the width of the South hand
  const biddingBoxWidth = southHandRect.width*444/345 ;
  biddingBox.style.position = 'absolute';
  biddingBox.style.left = `${leftPosition}px`;
  biddingBox.style.width = `${southHandRect.width}px`;
  biddingBox.style.height = `${southHandRect.height}px`;

  // Calculate the new bounding rectangle of the bidding box after setting the width
  const biddingBoxRect = biddingBox.getBoundingClientRect();

  // Center the bidding box horizontally and position it above the South hand
  //biddingBox.style.left = `${(window.innerWidth - biddingBoxRect.width) / 2}px`;
  biddingBox.style.top = `${southHandRect.top - biddingBoxRect.height - 10}px`;

  // Ensure the bidding box is visible
  biddingBox.style.visibility = 'visible';
  biddingBox.style.zIndex = '1000';
}


function waitForValidBid() {
  return new Promise((resolve) => {
    function checkBidAndResolve() {
      console.log('Checking bid in waitForValidBid');
      if (checkBid()) {
        console.log('Valid bid detected, resolving promise');
        resolve();
      } else {
        setTimeout(checkBidAndResolve, 100); // Check again after 100ms
      }
    }
    checkBidAndResolve();
  });
}
function makeCallShowBid(call) {
  if (passes === 3) {
    return; // Exit the function early if passes equals 3
  }
...
/*   if (level < 0 || strain < 0) return false;
  ...
  if(seatBid[bidSeqPoint]==0)   {
    // input bid and match with call
  }
  whosTurn++;
  if (whosTurn == 4) whosTurn = 0;
  ...
  if (level == 0) {
    if (strain == 1) {
      auctionTable.rows[row].cells[col].innerHTML = 'X'; // Double
    } else if (strain == 2) {
      auctionTable.rows[row].cells[col].innerHTML = 'XX'; // Redouble
    } else {
      auctionTable.rows[row].cells[col].innerHTML = calls[strain];
    }
  } else {
    auctionTable.rows[row].cells[col].innerHTML =
      "<font color='000000'>" + level + '</font>' + suitHTMLs[strain];
  ...
  // explainCall('', bidSeqPoint);
  auctionTable.rows[row].cells[col].style.visibility = 'visible';
  if (level == 0 && strain == 0 && bidSeqPoint > 2) {
    passes++;
...
let myturn = false; // Initial value of myturn
...
// Function to handle suit button clicks
function handleSpecialBidClick(bid) {
  currentBid = bid;
  if (checkBid()) {
    validBidEntered = true;
    myturn = false;
  } else {
    showTransientAnnouncement('Try again', 1000); // Show try again message
  }
}

function handleSuitClick(suit) {
  currentBid += suitMapping[suit]; // Append the suit to the numeric part
  if (checkBid()) {
    validBidEntered = true;
    myturn = false;
  } else {
...
// Function to check if the entered bid matches the expected bid
function checkBid() {
   if (currentBid.toUpperCase() === expectedBid.toUpperCase()) {
    // Remove event listeners for number, suit, and special bid buttons
    removeAllEventListeners();
    makeCallShowBid(bidSequence[bidSeqPoint]);
    // Move to the next expected bid
    bidSeqPoint++;
    if (bidSeqPoint < bidSequence.length) {
      expectedBid = bidSequence[bidSeqPoint];
    }
    else
      {showTransientAnnouncement('Bidding complete', 1000);
    ...
    // Reset currentBid for the next expected bid
    currentBid = '';
    return true;
  } else {
    // Entered bid is incorrect, reset currentBid
...
function addAllEventListeners() {
  ...
  document.querySelectorAll('.number').forEach(button => {
    button.addEventListener('click', () => {
        ...
        updateSuitButtons(); // Call updateSuitButtons here
        document.getElementById('suits-row').style.visibility = 'visible';
    });
  });

 
  // Event listeners for pass, double, and redouble buttons
  document.getElementById('pass').addEventListener('click', () => {
    updateSuitButtons(); // Update buttons after a pass
  });

  document.getElementById('dbl').addEventListener('click', () => {
    document.getElementById('dbl').style.display = 'none';
    document.getElementById('redbl').style.display = 'inline-block';
    updateSuitButtons(); // Update buttons after a double
  });

  document.getElementById('redbl').addEventListener('click', () => {
    if (isValidRedouble(bidSequence.slice(0, bidSeqPoint))) {
        document.getElementById('redbl').style.display = 'none';
        updateSuitButtons(); // Update buttons after a redouble
    } else {
        console.log('Cannot redouble after a previous redouble.');
    }
  });
  ...
  suitButtons.forEach(button => {
    button.addEventListener('click', () => handleSuitClick(button.dataset.value));
  });

//   // Add event listeners to Pass, Double, and Redouble buttons
  const passButton = document.getElementById('pass');
  const doubleButton = document.getElementById('dbl');
  const redoubleButton = document.getElementById('redbl');
  passButton.addEventListener('click', () => handleSpecialBidClick('P'));
  doubleButton.addEventListener('click', () => handleSpecialBidClick('D'));
  redoubleButton.addEventListener('click', () => handleSpecialBidClick('R'));
 }




// Function to remove event listeners from all buttons
function removeAllEventListeners() {
  // Remove event listeners from number buttons
  const numberButtons = document.querySelectorAll('.number');
  numberButtons.forEach(button => {
    button.removeEventListener('click', () => handleNumberClick(button.dataset.value));
  });

  // Remove event listeners from suit buttons
  const suitButtons = document.querySelectorAll('.suit');
  suitButtons.forEach(button => {
    button.removeEventListener('click', () => handleSuitClick(button.dataset.value));
  });

  // Remove event listeners from Pass, Double, and Redouble buttons
  if(passes<3)
  {
  const passButton = document.getElementById('pass');
  const doubleButton = document.getElementById('dbl');
  const redoubleButton = document.getElementById('redbl');
  passButton.removeEventListener('click', () => handleSpecialBidClick('Pass'));
  doubleButton.removeEventListener('click', () => handleSpecialBidClick('Double'));
  redoubleButton.removeEventListener('click', () => handleSpecialBidClick('Redouble'));
  }
}
...
// Async function to handle the next bid
async function nextBid() {
  let retVal;
  
  if (bidSeqPoint <= bidSeqMax) {
    while (bidSeqPoint <= bidSeqMax && passes < 3) {
      if (seatBid[bidSeqPoint] == 0) { // South's turn to bid
        showTransientAnnouncement('Enter your bid', 1000);
        expectedBid = bidSequence[bidSeqPoint];
        addAllEventListeners();
        lastBid = bidSequence[bidSeqPoint - 1];
        updateSuitButtons();
        // Wait for a valid bid
        await waitForValidBid();
        if (passes<3){
          retVal = makeCallShowBid(bidSequence[bidSeqPoint]);
        }
        bidSeqPoint++;
        validBidEntered = false;
        await nextBid(); // Continue to the next bid
        return retVal;
      }
      retVal = makeCallShowBid(bidSequence[bidSeqPoint]);
      bidSeqPoint++;
...
function updateSuitButtons() {
  ...
  // Reset number buttons visibility
  numberButtons.forEach(button => {
      button.style.display = 'inline-block';
  });

  // Find the last numeric bid
  let lastNumericBid = null;
  let lastNumericBidIndex = bidSeqPoint - 1;
  while (lastNumericBidIndex >= 0 && !isNumericBid(bidSequence[lastNumericBidIndex])) {
      lastNumericBidIndex--;
  }
  if (lastNumericBidIndex >= 0) {
      lastNumericBid = bidSequence[lastNumericBidIndex];
  }

  // If there was no previous numeric bid, show all number buttons
  if (!lastNumericBid) {
      suitButtons.forEach(button => {
          button.style.display = 'inline-block';
      });
  ...
  const lastNumericLevel = parseInt(lastNumericBid.slice(0, -1));
  const lastNumericSuit = lastNumericBid.slice(-1);

  // Hide number buttons below the last numeric level, except if the next valid bid is at the same level
  numberButtons.forEach(button => {
      const number = parseInt(button.getAttribute('data-value'));
      if (number < lastNumericLevel) {
          button.style.display = 'none';
      } else if (number === lastNumericLevel) {
          // Check if there is a valid bid at the same level
          const hasValidSameLevelBid = ['C', 'D', 'H', 'S', 'N'].some(suit => isValidBid(`${number}${suit}`));
          if (!hasValidSameLevelBid) {
              button.style.display = 'none';
          }
      }
  });
  ...
  suitButtons.forEach(button => {
      const suit = button.getAttribute('data-value');
      let bid;
      if (selectedNumber) {
          bid = `${selectedNumber}${getSuitLetter(suit)}`;
      } else {
          bid = `${lastNumericLevel + 1}${getSuitLetter(suit)}`;
      }

      if (isValidBid(bid)) {
          button.style.display = 'inline-block';

          // Check if there are any valid bids for the next level
          const nextLevel = parseInt(bid.slice(0, -1)) + 1;
          const hasValidNextLevelBid = ['C', 'D', 'H', 'S', 'N'].some(suit => isValidBid(`${nextLevel}${suit}`));

          // Hide number buttons if there are no valid bids for the next level
          numberButtons.forEach(numberButton => {
              const number = numberButton.getAttribute('data-value');
              if (number === nextLevel.toString() && !hasValidNextLevelBid) {
  ...
  if (bidSeqPoint === 0) {
      ...
      let bids = bidSequence.slice(0, bidSeqPoint);
      if (isValidDouble(bids)) {
          document.getElementById('dbl').style.display = 'inline-block';
          document.getElementById('dbl').style.visibility = 'visible';

         
      } else {
          document.getElementById('dbl').style.display = 'none';
      }
      if (isValidRedouble(bids)) {
          document.getElementById('redbl').style.display = 'inline-block';
          document.getElementById('redbl').style.visibility = 'visible';
...
function isValidBid(bid) {
  ...
  if (lastBid === 'P') {
      // If the last bid was a pass, find the last non-pass bid
      let lastNonPassBidIndex = bidSeqPoint - 2;
      while (
          lastNonPassBidIndex >= 0 &&
          (bidSequence[lastNonPassBidIndex] === 'P' ||
           bidSequence[lastNonPassBidIndex] === 'D' ||
           bidSequence[lastNonPassBidIndex] === 'R')
      ) {
          lastNonPassBidIndex--;
      }
      if (lastNonPassBidIndex >= 0) {
          const lastNonPassBid = bidSequence[lastNonPassBidIndex];
          const lastNonPassBidLevel = parseInt(lastNonPassBid.slice(0, -1));
          const lastNonPassBidSuit = lastNonPassBid.slice(-1);
          const bidLevel = parseInt(bid.slice(0, -1));
          const bidSuit = bid.slice(-1);

          if (bidLevel > lastNonPassBidLevel) {
              return true; // Higher level bid is always valid
          } else if (bidLevel === lastNonPassBidLevel) {
              const suitOrder = ['C', 'D', 'H', 'S', 'N'];
              const lastNonPassBidSuitIndex = suitOrder.indexOf(lastNonPassBidSuit);
              const bidSuitIndex = suitOrder.indexOf(bidSuit);
              return bidSuitIndex > lastNonPassBidSuitIndex; // Higher suit is valid at the same level
          } else {
              return false; // Lower level bid is not valid
          }
      } else {
          // If all previous bids were passes, any bid is valid
          return true;
      }
  } else if (lastBid === 'D' || lastBid === 'X') {
      // If the last bid was a double, find the bid before the double
      let lastNonDoubleBidIndex = bidSeqPoint - 2;
      while (lastNonDoubleBidIndex >= 0 && bidSequence[lastNonDoubleBidIndex] === 'P') {
          lastNonDoubleBidIndex--;
      }
      if (lastNonDoubleBidIndex >= 0) {
          const lastNonDoubleBid = bidSequence[lastNonDoubleBidIndex];
          const lastNonDoubleBidLevel = parseInt(lastNonDoubleBid.slice(0, -1));
          const lastNonDoubleBidSuit = lastNonDoubleBid.slice(-1);
          const bidLevel = parseInt(bid.slice(0, -1));
          const bidSuit = bid.slice(-1);
          if (bidLevel > lastNonDoubleBidLevel) {
              return true; // Higher level bid is always valid
          } else if (bidLevel === lastNonDoubleBidLevel) {
              const suitOrder = ['C', 'D', 'H', 'S', 'N'];
              const bidSuitIndex = suitOrder.indexOf(bidSuit);
              const lastNonDoubleBidSuitIndex = suitOrder.indexOf(lastNonDoubleBidSuit);
              return bidSuitIndex > lastNonDoubleBidSuitIndex; // Higher suit is valid at the same level
          } else {
              return false; // Lower level bid is not valid
          }
      } else {
          return true; // If there were no bids before the redouble, any numeric bid is valid
      }
  } else if (lastBid === 'XX' || lastBid === 'R') {
      // If the last bid was a redouble, find the previous non-redouble numeric or pass call
      let lastNonRedoubleCallIndex = bidSeqPoint - 2;
      while (
          lastNonRedoubleCallIndex >= 0 &&
          (bidSequence[lastNonRedoubleCallIndex] === 'P' ||
           bidSequence[lastNonRedoubleCallIndex] === 'D' ||
           bidSequence[lastNonRedoubleCallIndex] === 'X')
      ) {
          lastNonRedoubleCallIndex--;
      }
      if (lastNonRedoubleCallIndex >= 0) {
          const previousCall = bidSequence[lastNonRedoubleCallIndex];
          const previousCallLevel = parseInt(previousCall.slice(0, -1));
          const previousCallSuit = previousCall.slice(-1);
          const currentCallLevel = parseInt(bid.slice(0, -1));
          const currentCallSuit = bid.slice(-1);

          if (currentCallLevel > previousCallLevel) {
              return true; // Higher-level bids are always valid
          } else if (currentCallLevel === previousCallLevel) {
              const suitOrder = ['C', 'D', 'H', 'S', 'N'];
              return (
                  suitOrder.indexOf(currentCallSuit) >
                  suitOrder.indexOf(previousCallSuit)
              ); // Higher suits are valid at equal levels
          } else {
              return false; // Lower-level bids are invalid
          }
      } else {
          return true; // No prior numeric calls exist; any numeric call is valid.
      }
  } else {
      // Handle regular numeric bids or non-special cases with updated logic for NaN checks.
      if (isNaN(parseInt(lastBid.slice(0, -1)))) {
        return true; 
       } else {
        const lastBidLevel = parseInt(lastBid.slice(0, -1));
        const lastBidSuit = lastBid.slice(-1);
        const bidLevel = parseInt(bid.slice(0, -1));
        const bidSuit = bid.slice(-1);

        if (bidLevel > lastBidLevel) {
            return true; // Higher level bid is always valid
        } else if (bidLevel === lastBidLevel) {
            const suitOrder = ['C', 'D', 'H', 'S', 'N'];
            const lastBidSuitIndex = suitOrder.indexOf(lastBidSuit);
            const bidSuitIndex = suitOrder.indexOf(bidSuit);
            return bidSuitIndex > lastBidSuitIndex; // Higher suit is valid at the same level
        } else {
            return false; // Lower level bid is not valid
        }
       }
...
function isValidDouble(bids) {

  // Check the bid immediately before the current one (bidSeqPoint - 1)
  if (bidSeqPoint > 0) {
      let lastBid = bids[bidSeqPoint - 1];
      if (lastBid === 'D' || lastBid === 'R'){
          return false
      }
      if (isSuitBid(lastBid)) {
          return true; // Show double if the last bid was a suit bid
      } else {
          // Skip back two more bid places to check bidSeqPoint - 3
          if (bidSeqPoint > 2) {
              let prevPartBid = bids[bidSeqPoint - 2];
              if (prevPartBid === 'D' || prevPartBid === 'R'){
                 return false
             }
              let prevOppsBid = bids[bidSeqPoint - 3];
              if (isSuitBid(prevOppsBid)) {
                  return true; // Show double if the bidSeqPoint - 3 was a suit bid
              }
          }
      }
  }
  return false; // Do not show double otherwise
}

function isSuitBid(bid) {
  const suitBids = ['C', 'D', 'H', 'S', 'N'];
  return suitBids.includes(bid.slice(-1));
}

function isValidRedouble(bids) {
  // Check if the bid at bidSeqPoint - 1 is 'D'
  if (bidSeqPoint > 0 && bids[bidSeqPoint - 1] === 'D') {
      return true;
  }

  // Check if the bid at bidSeqPoint - 2 is a suit bid
  if (bidSeqPoint > 1 && isSuitBid(bids[bidSeqPoint - 2])) {
      return false;
  }

  if  (bids[bidSeqPoint - 2] ==='R') {
      return false;
  }

  // Check if the bid at bidSeqPoint - 3 is 'D'
  if (bidSeqPoint > 2 && bids[bidSeqPoint - 3] === 'D') {
      return true;
  }
